...
from .config import GPT2DiffuserConfig


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "jepagpt2"
_CONFIG_FOR_DOC = "GPT2DiffuserConfig"
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.ln_1 = AdaptiveLayerNorm(hidden_size)
        self.attn = GPT2Attention(config, layer_idx=layer_idx)
        self.ln_2 = AdaptiveLayerNorm(hidden_size)
    ...
    def forward(
        ...
        hidden_states = self.ln_1(hidden_states, scale_attn, shift_attn)
        ...
        # residual connection
        hidden_states = scale_attn * attn_output + residual
        ...
        residual = hidden_states
        hidden_states = self.ln_2(hidden_states, scale_mlp, shift_mlp)
        feed_forward_hidden_states = self.mlp(hidden_states)
        # residual connection
        hidden_states = residual + gate_mlp * feed_forward_hidden_states
...
class GPT2PreTrainedModel(PreTrainedModel):
    ...
    config_class = GPT2DiffuserConfig
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def _init_weights(self, module, *args, **kwargs):
        super()._init_weights(self, module, *args, **kwargs)
        for block in self.h:
...
@add_start_docstrings(
    """
    The GPT2 Model transformer with a JEPA language modeling head on top.
    """,
    GPT2_START_DOCSTRING,
)
class GPT2DiffuserLMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            noise = randn_tensor(inputs_embeds.shape, generator=None, device=self.device, dtype=self.dtype)
            noisy_inputs_embeds = self.noise_scheduler.add_noise(inputs_embeds, noise, timesteps)
            ...
            pred_x = transformer_outputs[0]

            if self.noise_scheduler.config.prediction_type == "epsilon":
                target_x = noise
            elif self.noise_scheduler.config.prediction_type == "v_prediction":
                target_x = self.noise_scheduler.get_velocity(inputs_embeds, noise, timesteps)
            else:
            ...
            input_embs_0 = self.noise_scheduler.add_noise(inputs_embeds, noise, timesteps)
            ...
            time_embs = self.timestep_embedding(torch.LongTensor(0).to(self.device))
            x_tp1_noise = randn_tensor((bsz, 1, inputs_embeds.shape[-1]), generator=None, device=self.device, dtype=self.dtype)
            position_ids = torch.arange(inputs_embeds.shape[1] + 1).long().to(inputs_embeds.device)[None, ...]
            self.noise_scheduler.set_timesteps(num_inference_steps)
            for t in self.noise_scheduler.timesteps:
                timesteps = torch.LongTensor([t]).to(self.device)
                time_embs = self.timestep_embedding(timesteps).unsqueeze(1)
                conditions = time_embs
                x_inputs = torch.concatenate([inputs_embeds, x_tp1_noise], dim=1)
                pred_x_tp1_noise = self.transformer(
                    inputs_embeds=x_inputs,
                    conditions=conditions,
                    past_key_values=past_key_values,
                    attention_mask=attention_mask,
                    token_type_ids=token_type_ids,
...
GPT2DiffuserLMHeadModel.register_for_auto_class("AutoModel")
GPT2DiffuserLMHeadModel.register_for_auto_class("AutoModelForCausalLM")
AutoModel.register(GPT2DiffuserConfig, GPT2DiffuserLMHeadModel)
AutoModelForCausalLM.register(GPT2DiffuserConfig, GPT2DiffuserLMHeadModel)