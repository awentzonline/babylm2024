...
from babylm.models.diffuse.config import VectorDiffusionConfig
...
class HoloDecoder(PreTrainedModel):
    config_class = VectorDiffusionConfig
...
class VectorDiffuser(PreTrainedModel):
    config_class = VectorDiffusionConfig
    ...
    def mup_base_shapes(cls, filename=None, base_kwargs=None, delta_kwargs=None):
        if not hasattr(cls, '_mup_base_shapes'):
            ...
            base_config = VectorDiffusionConfig(
                model_dims=128,
                **base_kwargs,
            )
